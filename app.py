...
def generate_key():
    ...
    return "QANH-" + r(10) + "-" + r(10) + "-" + r(10) + "-" + r(5)
...
def api_buy_key():
    ...
    key = generate_key()
    cmds = {'week': '1w', 'month': 'vip 1thang', 'forever': 'vip vinhvien'}
    try:
        requests.post('https://api.telegram.org/bot' + BOT_TOKEN + '/sendMessage',
            json={'chat_id': CHAT_ID, 'text': '/taokey ' + cmds.get(key_type, '1w')}, timeout=5)
    except:
        pass
    if key_type == 'week':
...
def api_get_free_link():
    ...
    callback_url = "https://shopvip-zykg.onrender.com/verify/" + token
    try:
        api_url = "https://link4m.co/api-shorten/v2?api=" + LINK4M_API_KEY + "&url=" + callback_url
...
def verify_page(token):
    ...
    data.setdefault("free_keys", {})[key] = {
        ...
        "email": pending_verify[token]["email"],
        "date": today,
        "verified": True
    }
    ...
    return "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Xac Thuc</title><style>body{background:#0f0c29;color:#fff;text-align:center;padding:50px;font-family:Arial}h2{color:#0f0}.key{color:#0f0;background:#000;padding:15px;border-radius:10px;border:1px dashed #0f0;font-family:monospace;font-size:18px}button{background:#0f0;color:#000;padding:12px 25px;border:none;border-radius:8px;font-weight:bold;font-size:16px;margin:10px}</style></head><body><h2>HOAN THANH NHIEM VU!</h2><div class='key'>" + key + "</div><button onclick=\"navigator.clipboard.writeText('" + key + "')\">COPY KEY</button><p style='color:#aaa'>Dung /kichhoat " + key[:20] + "... trong bot</p></body></html>"
...
var PARTNER_ID='70406595609';
var PARTNER_KEY='add9c7d61cb54ff1b447e2188c71a8c2';
...
currentUser={name:'Admin',email:'admin@qanhshop.com',password:pass,balance:999999999,isAdmin:true};
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=port)def generate_key():
    c = string.ascii_uppercase + string.digits
    r = lambda l: ''.join(secrets.choice(c) for _ in range(l))
